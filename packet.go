...
func ReadLayersV6(ip *layers.IPv6, tcp *layers.TCP, eth *layers.Ethernet) *packet_metadata {
		...
		if *IPv6 {
			...
			if ipLayer != nil {
				...
				if ethLayer != nil {
					...
					metapacket := ReadLayersV6(ip, tcp, eth)
					// fmt.Println(metapacket)
					return metapacket
				}
			}
		} else {
...
func SplitFromLastColon(input string) []string {
	lastIndex := strings.LastIndex(input, ":")
	if lastIndex == -1 {
		return []string{input}
	}
	left := input[:lastIndex]
	right := input[lastIndex+1:]
